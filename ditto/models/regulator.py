...
class Regulator(DiTToHasTraits):
    ...
    delay = Float(help='''The delay for first tap change operation''', default_value=None)
    highstep = Int(help='''The hightest possible tap step position from neutral''', default_value=None)
    lowstep = Int(help='''The lowest possible tap step position from neutral''', default_value=None)
    pt_ratio = Float(help='''The turns ratio used for the power transducer with a line-drop compensator.''', default_value=None)
    ct_ratio = Float(help='''The turns ratio used for the current transducer with a line-drop compensator.''', default_value=None)
    phase_shift = Float(help='''The degree phase shift that the regulator causes.''', default_value=None)
    ltc = Int(help='''1 if this regulator is a load tap changer, 0 otherwise.''', default_value=None)
    bandwidth = Float(help='''The band before a change occurs in the regulator''', default_value=None)
    bandcenter = Float(help='''This is the target value for the regulator. Should often be the nominal voltage''', default_value=None)
    voltage_limit = Float(help='''The maximum voltage allowed on the PT secondary''', default_value=None)
    from_element = Any(help='''The node which connects to the 'from' end of the regulator''', default_value=None)
