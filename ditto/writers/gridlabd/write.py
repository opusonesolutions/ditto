...
'''
        for i in model.models:
            if isinstance(i, Node):
                ...
                ## Modification made by Xiangqi Zhu on Apr 10, 2018
                # bus type
                if hasattr(i, 'bustype') and i.bustype is not None:
                    fp.write('     bustype {nv};\n'.format(nv=i.bustype))
                        ...
                        if hasattr(j,'phase') and j.phase is not None:
                            ...
                            if hasattr(j,'use_zip') and j.use_zip is not None:
                                if j.use_zip ==1: #This means that all the required values are not None
                                    ...
                                    if hasattr(j,'p') and j.p !=0 and hasattr(j,'q') and j.q !=0 and hasattr(j,'phase') and j.phase is not None:
                                        fp.write('    constant_power_{phase} {cp};\n'.format(phase=j.phase,cp=str(complex(j.p,j.q)).strip('()')))
                                    if hasattr(j,'p') and j.p !=0 and hasattr(j,'q') and j.q ==0 and hasattr(j,'phase') and j.phase is not None:
                                        fp.write('    constant_power_{phase} {flo};\n'.format(phase=j.phase, flo=str(j.p).strip('()')))
                                    if hasattr(j,'p') and j.p ==0 and hasattr(j,'q') and j.q !=0 and hasattr(j,'phase') and j.phase is not None:
                                        fp.write('    constant_power_{phase} {flo};\n'.format(phase=j.phase, flo=str(j.p)+'+'+str(j.q)+'j'.strip('()')))
    ...
    def write_transformer_configurations(self, model, fp):
        ...
        for i in model.models:
            if isinstance(i,PowerTransformer):
              ...
              #Modification by Xiangqi Zhu on Mar 5, 2018
                if hasattr(i,'impedance') and i.impedance is not None:
                    dic['impedance']=i.impedance
                if hasattr(i,'powerrating') and i.powerrating is not None:
                   dic['power_rating']=i.powerrating
                if hasattr(i,'primaryvoltage') and i.primaryvoltage is not None:
                    dic['primary_voltage']=i.primaryvoltage
                if hasattr(i,'secondaryvoltage') and i.secondaryvoltage is not None:
                    dic['secondary_voltage']=i.secondaryvoltage
                if hasattr(i,'connectiontype') and i.connectiontype is not None:
                    if  i.connectiontype=='YGYG':
                            dic['connect_type']='WYE_WYE'
                    if  i.connectiontype=='DD':
                            dic['connect_type']='DELTA_DELTA'
                    if  i.connectiontype=='DYG':
                            dic['connect_type']='DELTA_GWYE'
                if hasattr(i,'secondaryvoltage') and i.secondaryvoltage is not None:
                    dic['secondary_voltage']=i.secondaryvoltage
